...
// 트리 아이템 타입 정의
var BaekjoonItemType;
(function (BaekjoonItemType) {
    BaekjoonItemType[BaekjoonItemType["Category"] = 0] = "Category";
    BaekjoonItemType[BaekjoonItemType["Problem"] = 1] = "Problem";
})(BaekjoonItemType || (BaekjoonItemType = {}));
// 트리 아이템 클래스 정의
...
// 트리 뷰 프로바이더 클래스 정의
class BaekjoonTreeDataProvider {
    ...
    async getStepProblems(step) {
        if (this.problemCache.has(step)) {
            return this.problemCache.get(step);
        }
        try {
            const response = await axios_1.default.get(`https://www.acmicpc.net/step/${step}`, {
                headers: {
    ...
    async getAlgorithmProblems(category) {
        const categoryMap = {
            '구현': 102,
            '다이나믹 프로그래밍': 25,
            '그래프 이론': 7,
        ...
        try {
            const response = await axios_1.default.get(`https://www.acmicpc.net/problemset?sort=ac_desc&algo=${categoryId}&page=1`, {
                headers: {
                    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
                }
            });
        ...
        switch (element.label) {
            ...
            default:
                ...
                if (stepMatch) {
                    ...
                    return problems.map(problem => new BaekjoonTreeItem(`${problem.title} (${problem.id}번)`, vscode.TreeItemCollapsibleState.None, {
...
function activate(context) {
    ...
    // 문제 내용을 가져오는 함수
    async function fetchProblemContent(problemId) {
        try {
            const response = await axios_1.default.get(`https://www.acmicpc.net/problem/${problemId}`, {
                headers: {
                    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
                }
            });
    ...
    context.subscriptions.push(vscode.commands.registerCommand('baekjoon.refresh', () => {
        treeDataProvider.refresh();
        vscode.window.showInformationMessage('백준 문제 목록을 새로고침했습니다.');
    }), vscode.commands.registerCommand('baekjoon.openProblem', async (problemId) => {
        try {
            // 문제 내용 가져오기
            ...
            if (!problemEditor || problemEditor.document.isClosed) {
                ...
                const fullRange = new vscode.Range(problemEditor.document.positionAt(0), problemEditor.document.positionAt(problemEditor.document.getText().length));
